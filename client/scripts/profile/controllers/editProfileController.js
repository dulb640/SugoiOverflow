  ...
  .controller('editProfileController',
    function ($scope, $q, $routeParams, $location, $upload, $timeout, tagsDataService, profilesDataService, $localStorage, autocompleteService, changeCase) {
      ...
      function generateAvatarSrc () {
        $scope.avatarUrl = window.s.sprintf('/api/files/avatar/%s?%s', $scope.username, new Date().getTime())
      }
      ...
      window._.extend($scope, {
        submit: function () {
          var profile = {
            ...
            selectedTags: window._.pluck($scope.selectedTags, 'text')
          }
      ...
      profilesDataService.getCurrentUserProfile()
        .then(function (user) {
          ...
          $scope.selectedTags = window._.map(user.profile.selectedTags, function mapTags (tag) {
              return {
                text: tag
              }
            })
